...
export const ExportStore = signalStore(
  ...
  withComputed((store) => ({
    validTabs: computed(() => store.currentWindowTabs().filter(tab => DESIRED_URL_PREFIXES.some(prefix => tab.url?.includes(prefix)))),
  ...
  withMethods((store) => {
      ...
      return {
        ...
        exportTabs: rxMethod<ExportTabsRequest>(pipe(switchMap((request) => {
          return httpService.exportTabs(request)
        }), tapResponse(
          (response) => {
  ...
  withHooks({
    onInit: async (store) => {
      const chromeFacade = inject(ChromeFacadeService)
